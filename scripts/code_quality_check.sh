...
#   ./scripts/code_quality_check.sh --no-tests         # Skip pytest
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo "Usage: $0 [--all] [--files FILE1 FILE2 ...] [--no-type-check] [--no-tests] [--no-docstring-checks] [--verbose]" >&2
            exit 1
            ;;
    esac
done
...
echo "[1/8] Linting (includes imports + docstring style)..."
...
if [[ "$SKIP_DOCSTRING_CHECKS" == false && ${#SRC_FILES[@]} -gt 0 ]]; then
    ...
    echo "[3/8] Docstring freshness... (skipped)"
fi

# Step 4: Docstring Enrichment (missing sections audit)
if [[ "$SKIP_DOCSTRING_CHECKS" == false && ${#SRC_FILES[@]} -gt 0 ]]; then
    echo "[4/8] Docstring enrichment..."
    ...
    echo "[4/8] Docstring enrichment... (skipped)"
fi

# Step 5: Docs Coverage (missing __init__.py detection)
if [[ "$SKIP_DOCSTRING_CHECKS" == false && ${#SRC_FILES[@]} -gt 0 ]]; then
    echo "[5/8] Docs coverage..."
...
if [[ "$SKIP_DOCSTRING_CHECKS" == false && ${#SRC_FILES[@]} -gt 0 ]]; then
    ...
    echo "[6/8] Griffe docstring warnings... (skipped)"
fi
...
if [[ "$SKIP_TYPE_CHECK" == false ]]; then
    ...
    echo "[7/8] Type checking... (skipped)"
fi
...
if [[ "$SKIP_TESTS" == false ]]; then
    ...
    echo "[8/8] Running tests... (skipped)"
fi
...
TEST_FAILURES=$(echo "${TEST_FAILURES:-0}" | tr -d '[:space:]')
...
if [[ $TEST_FAILURES -gt 0 ]]; then
    echo "- Tests: Failures detected (see output above)"
fi

# Exit status - type errors, docs coverage, griffe issues, and test failures block; others are advisory
if [[ $TYPE_ERRORS -gt 0 ]]; then
    echo ""
    ...
    echo ""
    echo "Status: ⚠ Test failures require attention"
    exit 1
