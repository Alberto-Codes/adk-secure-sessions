...
def check_griffe_warnings(files: list[str] | None = None) -> list[str]:
    ...
    Args:
        files (list[str] | None): Optional list of files to check.
            If None, checks all of src/adk_secure_sessions.
    ...
    try:
        # Load the package
        pkg = griffe.load(
            "adk_secure_sessions", search_paths=["src"], docstring_parser="google"
        )
    ...
    if files:
        filtered = []
        for w in warnings:
            # Warning format: "src/adk_secure_sessions/api.py:1098: No type..."
            if ":" in w:
                warn_file = w.split(":")[0]
...
def main() -> int:
    ...
    parser.add_argument(
        "--files",
        nargs="*",
        help="Specific files to check (default: all src/adk_secure_sessions)",
    )
