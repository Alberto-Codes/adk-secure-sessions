...
- **Exception names**: end with "Error" suffix (e.g., `DecryptionError`)
...
Use maximum line length of **88 characters** (ruff formatter target). Use **4 spaces per indentation level**. Never use tabs.
...
Use `X | None` syntax for optional types. Always be explicit about None in type hints. Use `from __future__ import annotations` for forward references.

Do not annotate `self` or `cls` parameters.
...
Use implicit boolean evaluation for containers. Always use `is` or `is not` for None checks. Never compare booleans using `==` or `!=`.

## Functions and Methods

Keep functions focused and reasonably short. Never use mutable objects as default arguments.

## File and Resource Management

Always use `with` statements or async context managers for resource management.

## Exception Handling

Use specific exception types. Never use bare `except:`. Include informative error messages with context. Never include sensitive data (keys, ciphertext, plaintext) in exception messages.

## Comprehensions and Generators

Use list/dict/set comprehensions for simple transformations. Avoid multiple `for` clauses in a single comprehension.

## Async Conventions

All public APIs MUST be `async def`. Use `aiosqlite` for SQLite, async SQLAlchemy for other databases. Use `asyncio.to_thread()` for CPU-bound work that needs to avoid blocking the event loop.

## General Principles

Prioritize code readability and clarity over cleverness. Be consistent with existing code style when editing files. Run `ruff check` and `ruff format` for compliance.
